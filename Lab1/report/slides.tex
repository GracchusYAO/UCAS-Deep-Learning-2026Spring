...
\begin{frame}{结构设计分析}
	\begin{columns}[T,onlytextwidth]
		\begin{column}{0.49\textwidth}
			\begin{block}{为什么使用浅层 CNN}
				MNIST 图象分辨率小、背景简单，类别差异主要来自局部笔画和整体轮廓；两层卷积已经能够覆盖从边缘到笔画组合的特征层次。
			\end{block}
			\begin{itemize}
				\item $5\times5$ 卷积覆盖较宽局部笔画区域
				\item padding 2 保持空间尺寸，减少边缘信息丢失
				\item ReLU 引入非线性表达能力
			\end{itemize}
		\end{column}
		\begin{column}{0.49\textwidth}
			\begin{block}{池化与分类头}
				最大池化将 $28\rightarrow14\rightarrow7$，在压缩空间尺寸的同时增强小幅平移鲁棒性；全连接层利用粗略空间分布完成类别判别。
			\end{block}
			\begin{itemize}
				\item 第二层卷积组合更抽象的笔画结构
				\item 3136 维特征保留局部结构分布
				\item 参数主要集中在第一个全连接层
			\end{itemize}
		\end{column}
	\end{columns}
...
\begin{frame}{混淆矩阵分析}
	\begin{columns}[T,onlytextwidth]
		...
		\begin{column}{0.48\textwidth}
			\begin{block}{整体表现}
				测试集共 10000 张图象，正确分类 9933 张，错误分类 67 张，准确率为 \textbf{99.33\%}。
			\end{block}
			\begin{itemize}
				\item 混淆矩阵对角线占主导，各类别识别稳定
				\item 召回率相对最低的是数字 9，约为 98.61\%
				\item 最大混淆：$9\rightarrow4$，共 6 次
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{错误类型分析}
	\begin{columns}[T,onlytextwidth]
		\begin{column}{0.49\textwidth}
			...
			\end{block}
			\begin{itemize}
				\item $9\rightarrow4$：6 次
				\item $5\rightarrow3$：5 次
				\item $9\rightarrow5$：5 次
				\item $7\rightarrow2$：4 次
			\end{itemize}
		\end{column}
		\begin{column}{0.49\textwidth}
			\begin{block}{原因解释}
			...
			\begin{itemize}
				\item 错误具有可解释的形态相似性
				\item 当前模型没有表现出明显类别偏置
				\item 可通过轻量几何增强继续改善鲁棒性
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
